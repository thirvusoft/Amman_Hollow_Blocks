...
class User(Document):
	...
	def reset_password(self, send_email=False, password_expired=False):
		...
		link = get_url(url)
		if send_email:
			self.password_reset_mail(link, key= key if self.is_mobile_app_user else None)  # thirvusoft changes
	...
	def password_reset_mail(self, link, key=None):
		self.send_login_mail(_("Password Reset"), "password_reset", {"link": link, 'key': key}, now=True)  # thirvusoft changes
	...
	def find_by_credentials(cls, user_name: str, password: str, validate_password: bool = True):
		...
		if user and frappe.db.get_value("User", user.get('name'), "is_mobile_app_user") and not frappe.flags.mobile_login:  # thirvusoft changes
			return  # thirvusoft changes
